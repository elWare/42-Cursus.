...
int ft_popen(const char *file, char *const argv[], char type)
{
    ...
    int		pipe_fds[2];  // Array para los extremos del pipe
	__pid_t	pid;
    ...
    /*
     * CREAR PIPE:
     * - pipe_fds[0]: extremo de lectura (read end)
     * - pipe_fds[1]: extremo de escritura (write end)
     * 
     * El pipe permite comunicación unidireccional:
     * datos escritos en pipe_fds[1] pueden leerse desde pipe_fds[0]
     */
    if (pipe(pipe_fds) == -1)
        return -1;
