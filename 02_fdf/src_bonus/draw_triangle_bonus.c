...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/28 09:29:31 by jumarque          #+#    #+#             */
/*   Updated: 2025/03/24 10:58:21 by jumarque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_fillbottri(t_coord *p1, t_coord *p2, t_coord *p3, t_env *e)
{
	t_coord		p4;
	t_coord		p5;
	int			x;
...
void	ft_filltoptri(t_coord *p1, t_coord *p2, t_coord *p3, t_env *e)
{
	t_coord		p4;
	t_coord		p5;
	int			x;
...
void	ft_drawtriangle(t_coord *p1, t_coord *p2, t_coord *p3, t_env *e)
{
	...
	{
		if (p2->x == p3->x)
			ft_fillbottri(p1, p2, p3, e);
		else if (p1->x == p2->x)
			ft_filltoptri(p1, p2, p3, e);
		else
		{
			...
			p4.z = (p1->z + ((p2->x - p1->x) / (p3->x - p1->x)) * \
					(p3->z - p1->z));
			ft_fillbottri(p1, p2, &p4, e);
			ft_filltoptri(p2, &p4, p3, e);
		}
	}
}
